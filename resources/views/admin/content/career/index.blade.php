                                ...
                                <tr>
                                    ...
                                    <td>
                                        <small>
                                            ...
                                            <a href="{{ route('admin.content.career.update.status', $item->id) }}"><span class="{{ $item->is_active == 1 ? 'badge ms-1 bg-light-success' : 'badge ms-1 bg-light-danger' }}"><i class="bi {{ $item->is_active == 1 ? 'bi-check-lg' : 'bi-x-lg' }}"></i></span></a>
                                        </small><br>
                                        ...
                                        <small>Karir Kategori: {{ $item->careerCategory->name }} - {{ $item->careerCategory->career->name }}</small>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group" role="group">
                                            <a class="btn btn-primary" href="{{ route('admin.content.career.detail.index', $item->id) }}"><i class="bi bi-eye-fill"></i></a>
                                            <button type="button" class="btn btn-warning text-white" data-bs-toggle="modal"
                                                data-bs-target="#modal-form"
                                                onclick="openFormDialog('modal-form', 'edit', '{{ $item->id }}', '{{ $item->title }}', '{{ $item->description }}', '{{ $item->is_active }}', '{{ $item->featured_image }}', '{{ $item->careerCategory->career->id }}', '{{ $item->career_category_id }}', '{{ $item->started_at }}', '{{ $item->ended_at }}')">
                                                <i class="bi bi-pencil-fill"></i>
                                            </button>
                                            <button type="button" class="btn btn-danger"
                                                onclick="deleteDialog('{{ route('admin.content.career.destroy', $item->id) }}')">
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
            <div class="modal-content rounded shadow">
                ...
                <div class="modal-body">
                    <form class="form" id="form-modal" action="{{ route('admin.content.career.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-md-6 col-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-select" id="career" name="career_id" required>
                                ...
                                <div class="form-group">
                                    <label>Kategori Karir<span class="text-danger fw-bold">*</span></label>
                                    <select class="form-select" id="career-category" name="career_category_id" required>
                                    </select>
                                    <span class="text-danger" id="no-data-message" style="display: none;">No data found for selected career.</span>
                                </div>
                            </div>
                            <div class="col-md-6 col-12">
                                <div class="form-group">
    ...
    <script>
        $(document).ready(function () {
            $('#career').on('change', function () {
                ...
                $.ajax({
                    url: "{{ route('admin.api.fetch-career-category') }}",
                    type: "POST",
                    data: {
                        career_id: careerId,
                        _token: '{{csrf_token()}}'
                    ...
                    success: function (result) {
                        if (result.listCareerCategory.length === 0) {
                            $("#career-category").html('<option disabled selected hidden>No data found</option>');
                            $("#no-data-message").show();
                        } else {
                            $('#career-category').html('<option value="" disabled selected hidden>Pilih Kategori Karir</option>');
                            $.each(result.listCareerCategory, function (key, value) {
                                $("#career-category").append('<option value="' + value.id + '">' + value.name + '</option>');
                            });
                        }
                    },
    ...
    <script>
        ...
        function saveForm() {
            const requiredInputs = [
                ...
                { name: 'career_category_id', label: 'Kategori Karir' },
            ];
        ...
        function openFormDialog(target, type, id, title, description, is_active, featured_image, career_id, career_category_id, started_at, ended_at) {
            if (type === 'add') {
                $('#' + target + ' form').attr('action', '{{ route('admin.content.career.store') }}');
                ...
                $('#' + target + ' select[name="career_id"]').attr('required', 'required');
                $('#' + target + ' select[name="career_category_id"]').attr('required', 'required');
                $('#' + target + ' input[name="started_at"]');
                $('#' + target + ' input[name="ended_at"]');
            } else if (type === 'edit') {
                $('#' + target + ' .clear-after').val('');
                $('#' + target + ' form').attr('action', '{{ route('admin.content.career.update') }}');
                $('#' + target + ' .clear-after[name="id"]').val(id);
                $('#' + target + ' input[name="title"]').val(title);
                $('#' + target + ' select[name="is_active"]').val(is_active);
                $('#' + target + ' select[name="career_id"]').val(career_id);
                $('#' + target + ' select[name="career_category_id"]').val(career_category_id);
