...
@section('container')
            ...
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('admin.content.career.index') }}">Karir</a></li>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group" role="group">
                                            <button type="button" class="btn btn-warning text-white" data-bs-toggle="modal"
                                                ...
                                                onclick="openFormDialog('modal-form', 'edit', '{{ $item->id }}', '{{ $item->image }}', '{{ $item->career_article_id }}')">
                                                <i class="bi bi-pencil-fill"></i>
                                            </button>
                                            <button type="button" class="btn btn-danger"
                                                onclick="deleteDialog('{{ route('admin.content.career.detail.destroy', ['career_article_id' => $item->career_article_id, 'id' => $item->id]) }}')">
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
            <div class="modal-content rounded shadow">
                ...
                <div class="modal-body">
                    <form class="form" id="form-modal" action="{{ route('admin.content.career.detail.store', ['career_article_id' => 'career_article_id']) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-12 col-12">
                                <div class="form-group">
                                    ...
                                    <input class="form-control clear-after" type="hidden" name="career_article_id">
    ...
    <script>
        function openFormDialog(target, type, id, image, career_article_id) {
            if (type === 'add') {
                $('#' + target + ' form').attr('action', '{{ route('admin.content.career.detail.store', ['career_article_id' => 'career_article_id']) }}');
                $('#' + target + ' .form-control').val('');
                $('#' + target + ' input[name="image"]').attr('required', 'required');
                $('#' + target + ' input[name="career_article_id"]').val(id);
            } else if (type === 'edit') {
                const editUrl = '{{ url('admin/konten/karir/detil') }}' + '/' + career_article_id + '/update';
                $('#' + target + ' .clear-after').val('');
                $('#' + target + ' form').attr('action', editUrl);
                $('#' + target + ' .clear-after[name="id"]').val(id);
                $('#' + target + ' input[name="career_article_id"]').val(career_article_id);
            }
            $('#' + target).modal('toggle');
