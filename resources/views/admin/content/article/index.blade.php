                                ...
                                <tr>
                                    ...
                                    <td>
                                        <small>
                                            ...
                                            <a href="{{ route('admin.content.article.update.status', $item->id) }}"><span class="{{ $item->is_active == 1 ? 'badge ms-1 bg-light-success' : 'badge ms-1 bg-light-danger' }}"><i class="bi {{ $item->is_active == 1 ? 'bi-check-lg' : 'bi-x-lg' }}"></i></span></a>
                                        </small><br>
                                        {{ Str::limit($item->title, 75) }}<br>
                                        <small>Kategori: {{ $item->articleCategory->name }}</small>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group" role="group">
                                            <a class="btn btn-primary" href="{{ route('admin.content.article.detail.index', $item->id) }}"><i class="bi bi-eye-fill"></i></a>
                                            <button type="button" class="btn btn-warning text-white" data-bs-toggle="modal"
                                                data-bs-target="#modal-form"
                                                onclick="openFormDialog('modal-form', 'edit', '{{ $item->id }}', '{{ $item->title }}', '{{ $item->description }}', '{{ $item->is_active }}', '{{ $item->featured_image }}', '{{ $item->article_category_id }}')">
                                                <i class="bi bi-pencil-fill"></i>
                                            </button>
                                            <button type="button" class="btn btn-danger"
                                                onclick="deleteDialog('{{ route('admin.content.article.destroy', $item->id) }}')">
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
            <div class="modal-content rounded shadow">
                ...
                <div class="modal-body">
                    <form class="form" id="form-modal" action="{{ route('admin.content.article.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-md-6 col-12">
                                <div class="form-group">
                                    <label>Kategori Artikel<span class="text-danger fw-bold">*</span></label>
                                    <select class="form-select" name="article_category_id" required>
                                        <option value="" disabled selected hidden>Pilih Kategori</option>
                                        @foreach ($listArticleCategory as $item)
    ...
    <script>
        ...
        function saveForm() {
            const requiredInputs = [
                ...
                { name: 'article_category_id', label: 'Kategori Artikel' },
            ];
        ...
        function openFormDialog(target, type, id, title, description, is_active, featured_image, article_category_id) {
            if (type === 'add') {
                $('#' + target + ' form').attr('action', '{{ route('admin.content.article.store') }}');
                ...
                $('#' + target + ' select[name="article_category_id"]').attr('required', 'required');
            } else if (type === 'edit') {
                $('#' + target + ' .clear-after').val('');
                $('#' + target + ' form').attr('action', '{{ route('admin.content.article.update') }}');
                $('#' + target + ' .clear-after[name="id"]').val(id);
                $('#' + target + ' input[name="title"]').val(title);
                $('#' + target + ' select[name="is_active"]').val(is_active);
                $('#' + target + ' select[name="article_category_id"]').val(article_category_id);
