                                ...
                                <tr>
                                    ...
                                    <td>
                                        <small>
                                            ...
                                            <a href="{{ route('admin.content.program.update.status', $item->id) }}"><span class="{{ $item->is_active == 1 ? 'badge ms-1 bg-light-success' : 'badge ms-1 bg-light-danger' }}"><i class="bi {{ $item->is_active == 1 ? 'bi-check-lg' : 'bi-x-lg' }}"></i></span></a>
                                        </small><br>
                                        ...
                                        <small>Layanan: {{ $item->program->name }} - {{ $item->program->service->name }}</small>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group" role="group">
                                            <a class="btn btn-primary" href="{{ route('admin.content.program.detail.index', $item->id) }}"><i class="bi bi-eye-fill"></i></a>
                                            <button type="button" class="btn btn-warning text-white" data-bs-toggle="modal"
                                                data-bs-target="#modal-form"
                                                onclick="openFormDialog('modal-form', 'edit', '{{ $item->id }}', '{{ $item->title }}', '{{ $item->description }}', '{{ $item->is_active }}', '{{ $item->featured_image }}', '{{ $item->program->service->id }}', '{{ $item->program_id }}', '{{ $item->started_at }}', '{{ $item->ended_at }}', '{{ $item->batch }}', '{{ $item->price }}')">
                                                <i class="bi bi-pencil-fill"></i>
                                            </button>
                                            <button type="button" class="btn btn-danger"
                                                onclick="deleteDialog('{{ route('admin.content.program.destroy', $item->id) }}')">
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
            <div class="modal-content rounded shadow">
                ...
                <div class="modal-body">
                    <form class="form" id="form-modal" action="{{ route('admin.content.program.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-md-6 col-12">
                                ...
                                <div class="form-group">
                                    <label>Program<span class="text-danger fw-bold">*</span></label>
                                    <select class="form-select" id="program" name="program_id" required>
                                    </select>
                                    <span class="text-danger" id="no-data-message" style="display: none;">No data found for selected service.</span>
                                </div>
                            </div>
                            <div class="col-md-6 col-12">
                                <div class="form-group">
                                    <label>Tanggal Mulai<span class="text-danger fw-bold">*</span></label>
                                    <input type="date" class="form-control" name="started_at">
                                </div>
                            </div>
                            <div class="col-md-6 col-12">
                                <div class="form-group">
                                    <label>Tanggal Selesai<span class="text-danger fw-bold">*</span></label>
    ...
    <script>
        $(document).ready(function () {
            $('#service').on('change', function () {
                ...
                $.ajax({
                    url: "{{ route('admin.api.fetch-program') }}",
                    type: "POST",
                    data: {
                    ...
                    success: function (result) {
                        if (result.listProgram.length === 0) {
                            $("#program").html('<option disabled selected hidden>No data found</option>');
                            $("#no-data-message").show();
                        } else {
                            $('#program').html('<option value="" disabled selected hidden>Pilih Program</option>');
                            $.each(result.listProgram, function (key, value) {
    ...
    <script>
        ...
        function saveForm() {
            const requiredInputs = [
                ...
                { name: 'started_at', label: 'Tanggal Mulai' },
                { name: 'ended_at', label: 'Tanggal Selesai' },
        ...
        function openFormDialog(target, type, id, title, description, is_active, featured_image, service_id, program_id, started_at, ended_at, batch, price) {
            if (type === 'add') {
                $('#' + target + ' form').attr('action', '{{ route('admin.content.program.store') }}');
                ...
                $('#' + target + ' select[name="program_id"]').attr('required', 'required');
                ...
                $('#' + target + ' form').attr('action', '{{ route('admin.content.program.update') }}');
                ...
                $('#' + target + ' select[name="program_id"]').val(program_id);
