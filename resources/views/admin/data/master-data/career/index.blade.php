...
@section('container')
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group" role="group">
                                            <a class="btn btn-primary" href="{{ route('admin.data.master-data.career.detail.index', $item->id) }}"><i class="bi bi-eye-fill"></i></a>
                                            <button type="button" class="btn btn-warning text-white" data-bs-toggle="modal"
                                                data-bs-target="#modal-form"
                                                onclick="openFormDialog('modal-form', 'edit', '{{ $item->id }}', '{{ $item->name }}')">
                                                <i class="bi bi-pencil-fill"></i>
                                            </button>
                                            <button type="button" class="btn btn-danger"
                                                onclick="deleteDialog('{{ route('admin.data.master-data.career.destroy', $item->id) }}')">
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
            <div class="modal-content rounded shadow">
                ...
                <div class="modal-body">
                    <form class="form" id="form-modal" action="{{ route('admin.data.master-data.career.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
    ...
    <script>
        function openFormDialog(target, type, id, name) {
            if (type === 'add') {
                $('#' + target + ' form').attr('action', '{{ route('admin.data.master-data.career.store') }}');
                $('#' + target + ' .form-control').val('');
                $('#' + target + ' input[name="name"]').attr('required', 'required');
            } else if (type === 'edit') {
                $('#' + target + ' .clear-after').val('');
                $('#' + target + ' form').attr('action', '{{ route('admin.data.master-data.career.update') }}');
