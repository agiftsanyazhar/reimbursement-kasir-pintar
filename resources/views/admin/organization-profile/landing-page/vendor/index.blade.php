...
@section('container')
                            ...
                            @foreach ($vendor as $item)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <p class="text-center mt-3 fw-bold">{{ $item->name ?? '-' }}</p>
                                    </td>
                                    <td class="text-center">
                                        <div class="btn-group" role="group">
                                            <button type="button" class="btn btn-warning text-white" data-bs-toggle="modal"
                                                data-bs-target="#modal-form"
                                                onclick="openFormDialog('modal-form', 'edit', '{{ $item->id }}', '{{ $item->image }}', '{{ $item->name }}')">
                                                <i class="bi bi-pencil-fill"></i>
                                            </button>
                                            <button type="button" class="btn btn-danger"
                                                onclick="deleteDialog('{{ route('admin.organization-profile.landing-page.vendor.destroy', $item->id) }}')">
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
            <div class="modal-content rounded shadow">
                ...
                <div class="modal-body">
                    <form class="form" id="form-modal" action="{{ route('admin.organization-profile.landing-page.vendor.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-md-12 col-12">
                                <div class="form-group">
                                    <label>Foto<span class="text-danger fw-bold">*</span></label>
    ...
    <script>
        function openFormDialog(target, type, id, image, name) {
            if (type === 'add') {
                $('#' + target + ' form').attr('action', '{{ route('admin.organization-profile.landing-page.vendor.store') }}');
                ...
                $('#' + target + ' form').attr('action', '{{ route('admin.organization-profile.landing-page.vendor.update') }}');
